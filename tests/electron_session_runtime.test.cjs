...
function loadSessionRuntime() {
  return import(
    pathToFileURL(
      path.join(
        __dirname,
        "../desktop/electron/renderer/src/lib/session-runtime.mjs",
      ),
    ).href
  );
}
...
test("run.failed records the error and ends the run", async () => {
  ...
  assert.equal(state.messages.at(-1).text, "model exploded");
});
...
test("an event only touches the session it names", async () => {
  ...
  assert.equal(states.a.messages[0].text, "from a");
  assert.equal(states.b.messages[0].text, "from b");
});
...
test("interleaved runs keep independent transcripts and running flags", async () => {
  ...
  assert.deepEqual(
    states.a.messages.map((message) => message.text),
    ["answer a"],
  );
  ...
  assert.equal(states.b.messages[0].state, "done");
  assert.equal(states.b.messages[1].text, "answer b");
});

test("events without a session id are ignored", async () => {
  const { reduceSessions } = await loadSessionRuntime();

  const states = { a: { messages: [] } };
  assert.equal(
