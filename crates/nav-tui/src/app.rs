...
pub async fn run(
    ...
    let permissions = PermissionContext {
        ...
        // Placeholders. The real per-turn signal + steering queue are set
        // just before each `spawn_turn` call below so a trip or pending
        // message on turn N can't leak into turn N+1.
