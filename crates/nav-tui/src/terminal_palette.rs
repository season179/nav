...
pub fn best_color(target: (u8, u8, u8)) -> Color {
    match stdout_color_level() {
        ...
        // `supports-color` can under-report capability; RGB sequences are still
        // widely accepted, and Reset would make theme surfaces invisible.
        StdoutColorLevel::Ansi16 | StdoutColorLevel::Unknown => rgb_color(target),
    }
}
...
/// Probes OSC 10/11 default colors after raw mode is enabled, with a bounded timeout.
#[cfg(unix)]
pub(crate) fn probe_default_colors_at_startup() {
    let probed = crate::terminal_probe::default_colors(crate::terminal_probe::DEFAULT_TIMEOUT)
        .ok()
        .flatten();
    set_default_colors_from_startup_probe(probed);
}
...
mod tests {
    ...
    #[test]
    fn best_color_never_returns_reset_for_theme_surfaces() {
        assert_ne!(best_color((38, 38, 48)), Color::default());
        assert_ne!(best_color((30, 30, 36)), Color::default());
    }
}
