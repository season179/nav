...
async fn overflow_one_shot_recovery_trims_and_continues() {
    ...
    let turn_two = vec![StubItem::Err(crate::responses::ResponsesError::ContextWindowExceeded {
    ...
    assert_eq!(bodies.len(), 3, "agent should make exactly 3 transport calls");
...
async fn overflow_second_failure_surfaces_clean_error() {
    ...
    let turn_three = vec![StubItem::Err(crate::responses::ResponsesError::ContextWindowExceeded {
