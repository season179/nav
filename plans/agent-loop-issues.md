...
| # | Issue | Model | Scope |
|---|---|---|---|
| APR-01 | Guardrails policy engine | strong | Tool-name + args-based "requires_approval" decision. **Acceptance includes an explicit policy table per (tool × preset × arg class)**, not "default policy" handwave: `read`/`ls`/`ripgrep` auto-allow; `write`/`edit` auto-allow in-workspace, confirm otherwise; `bash` always confirm in v1. |
