...
*   **Provider Envelope Journal:** The canonical ledger is not the only evidence of what happened. Raw OpenAI Chat Completions, OpenAI Responses, ChatGPT subscription/Codex, Anthropic, and gateway request/response/stream payloads are stored as provider payload artifacts with decoder provenance. This lets nav re-decode old runs when an API schema or decoder changes.
